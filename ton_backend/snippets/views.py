...
class SnippetsList(ListCreateAPIView):
    queryset = Snippet.objects.all()
    serializer_class = SnippetSerializer
...
class SnippetDetails(RetrieveUpdateDestroyAPIView):
    queryset = Snippet.objects.all()
    serializer_class = SnippetSerializer
    permission_classes = [
...
""" @api_view(['GET', 'POST'])
def snippets_list(request: HttpRequest, format=None):
...
def snippet_details(request: HttpRequest, pk: int, format=None):
    try:
        snippet = Snippet.objects.get(pk=pk)
    except Snippet.DoesNotExist:
        return Response(status=status.HTTP_404_NOT_FOUND)
